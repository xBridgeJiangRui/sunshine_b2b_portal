...
");
    ...
    function check_module_result($limit, $start, $check_status, $q_doc_from_to, $q_exp_from_to, $q_refno, $q_period_code, $loc, $check_module, $dir, $order)
    {
        ...
        if ($check_module == 'panda_return_collection') {
            if (in_array('IAVA', $_SESSION['module_code'])) {
                $result = $this->db->query("SELECT a.customer_guid, IFNULL(a.b2b_dn_refno, '') AS prdn_refno, batch_no, location, doc_date, expiry_date, sup_code, sup_name, canceled, IF( STATUS = '0', 'Pending Accept', IF( STATUS = '1', 'Accepted', IF( STATUS = '2', 'Pending PRDN', IF( STATUS = '3', 'PRDN generated', IF( STATUS = '8', 'Amended', IF(STATUS = '9', 'Cancel', 'No Status' ) ) ) ) ) ) AS status_desc, status, accepted_at, accepted_by, cancel_remark, IF( STATUS NOT IN ('8','9') ,uploaded_image, '0') AS uploaded_image, b.b2b_registration, a.srb_accept_days, DATE_ADD(a.doc_date, INTERVAL a.srb_accept_days DAY) AS new_expiry_date FROM b2b_summary.dbnote_batch AS a INNER JOIN b2b_summary.supcus AS b ON a.sup_code = b.`code` AND a.customer_guid = b.customer_guid AND b.b2b_registration = '1' WHERE a.customer_guid  = '" . $_SESSION['customer_guid'] . "' AND a.location IN ($loc)    $q_doc_from_to $q_exp_from_to $q_refno  and status IN ($check_status) $q_period_code   ORDER BY $order  limit $start,$limit");
    ...
    function edi_refno_list($supplier_guid, $customer_guid, $edi_batch_no, $limit, $start, $col, $dir, $search)
    {
        if ($col == '' && $dir == '' && $start == '' && $limit == '') {
            $order_by = '';
        } else {
    ...
    //created by jr
    function edi_grn_tb($edi_batch_no, $status, $generate_date_from, $generate_date_to, $period_code, $supplier_guid, $customer_name, $type, $limit, $start, $col, $dir, $search)
    {
        if ($col == '' && $dir == '' && $start == '' && $limit == '') {
            $order_by = '';
        } else {
        ...
        $query = $this->db->query("SELECT a.`guid`,a.`edi_batch_no`,a.status,a.`type`,a.`file_name`,a.`refno`,b.`acc_name`,b.acc_guid,c.`supplier_name`,a.`created_at`,a.`updated_at`,a.`error_message_reason`,a.`refno`,a.`remark`,a.`updated_at`,a.`updated_by`
        FROM lite_b2b.`edi_log` AS a
        LEFT JOIN lite_b2b.`acc` AS b
        ON a.`customer_guid` = b.`acc_guid`
        LEFT JOIN lite_b2b.`set_supplier` AS c
