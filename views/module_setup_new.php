    ...
    columns: [
              {"data":"user_guid", render:function( data, type, row ){
                ...
                ?>
                  ...
                  element += '<button title="Edit" onclick="edit_user()" type="button" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#user" data-query="SELECT * FROM set_user where user_guid = \''+row['user_guid']+'\' " data-guid="'+row['user_guid']+'" data-name="'+row['user_name']+'" data-id="'+row['user_id']+'" data-password="'+row['user_password']+'" data-isactive="'+row['isactive']+'" data-user_group="'+row['user_group_name']+'" data-limited_location="'+row['limited_location']+'" data-user_group_guid="'+row['user_group_guid']+'"> <i class="glyphicon glyphicon-pencil"></i></button>';
