...
class general extends CI_Controller
{
    ...
    public function view_status()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $this->session->userdata('user_logs') == $this->panda->validate_login()) {
            ...
            $this->session->set_flashdata('message', 'ERROR CODE ER01. Please relogin');
            redirect('#');
        }
    }
    ...
    public function view_table()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $this->session->userdata('user_logs') == $this->panda->validate_login()) {
            ...
            if ($refno == '') {
                ...
                } else if ($check_module == 'panda_pci' || $check_module == 'panda_di') {
                    $query_refno = " and inv_refno = '$refno'";
                } else {
            ...
            if ($exp_to != '') {
                ...
                if ($check_module == 'panda_gr') {
                    $query_exp_from_to = " and z.inv_date between '$exp_from' and '$exp_to'";
                };
                if ($check_module == 'panda_return_collection') {
                    $query_exp_from_to = " and expiry_date between '$exp_from' and '$exp_to'";
                };
            ...
            if ($check_module == 'panda_gr') {
                ...
                if ($check_status == '') {
                    ...
                    $check_status = implode(',', array_filter($check_stat));
                }else if ($check_status == 'geinv')
                {
                    $check_status = "'','printed','viewed'";
                }
                else {
                    $check_status = "'" . $check_status . "'";
                }
            ...
            if ($check_module == 'panda_grda') {
                ...
                if ($check_status == '') {
                    //$check_status = "'','GQV','GRV', 'IAV', 'GDS','GDV'";
            ...
            if ($check_module == 'panda_pdncn') {
                ...
                if ($check_status == '') {
                    $check_status = "'','PDNAMT','PCNAMT'";
                } elseif ($check_status == 'ALL') {
                    $get_stat = $this->db->query("SELECT code from set_setting where module_name = 'PDNCN_FILTER_TYPE'");
            ...
            if ($check_module == 'panda_pci') {
                ...
                if ($check_status == '') {
                    ...
                    $get_stat = $this->db->query("SELECT code from set_setting where module_name = 'PCI_FILTER_STATUS'");
            ...
            if ($check_module == 'panda_di') {
                ...
                if ($check_status == '') {
                    ...
                    $get_stat = $this->db->query("SELECT code from set_setting where module_name = 'DI_FILTER_STATUS'");
            ...
            if ($check_module == 'panda_return_collection') {
                if ($check_status == '') {
                    $check_status = "'','0','1','2','3','4','8','9'"; // edit '4' // edit 8 = disupute , 9 = cancel, removed -1 2022/02/28
                }
                $columns = array(
                    0 => 'batch_no',
                    1 => 'location',
            ...
            if($check_module == 'panda_return_collection') 
            {
                if($order == 'batch_no')
                {
                    $order = "FIELD( a.status, '0', '1', '2', '3' , '9' , '8' ) ASC , a.doc_date ASC";
                }
                else
                {
                    $order = $order.' '.$dir;
            ...
            // $test_user_guid = $_SESSION['user_guid'];
            // if($test_user_guid == '7BA14C79BDDB11EBB0C4000D3AA2838A')
            // {
            //     echo $this->db->last_query(); die;
            // }
            $data = array();
            if (!empty($posts)) {
                ...
                foreach ($posts as $post) {
                    if ($check_module == 'panda_po_2') {
                        ...
                        if (in_array('HFSP', $_SESSION['module_code']) && $_REQUEST['status'] == '' && ($this->session->userdata('customer_guid') != '8D5B38E931FA11E79E7E33210BD612D3' && $this->session->userdata('customer_guid') != 'B00CA0BE403611EBA2FC000D3AC8DFD7'))
                        {
                    ...
                    if ($check_module == 'panda_gr') {
                        ...
                        if($this->session->userdata('customer_guid') == '907FAFE053F011EB8099063B6ABE2862')
                        {
                            $nestedData['refno'] = $post->refno;
                            $nestedData['view_po_refno'] = $post->refno;
                        }
                        else
                        {
                            $nestedData['refno'] = $post->refno;
                            $nestedData['view_po_refno'] = '<span style="display:flex;">' . $post->refno . '<i data-toggle="tooltip" data-placement="top" title="Click to preview PO details" class="fa fa-info-circle" style="padding-top:5px;padding-left:10px;cursor: pointer;"  id="view_po_data" gr_refno=' . $post->refno . '></i></span>';
                        }
                        ...
                        $nestedData['e_invdate'] = $post->einvdate;
                        ...
                        $nestedData['box'] = '<input type="checkbox" class="data-check" value="' . $post->refno . '" grda_status="' . $post->grda_status . '" doc_status="' . $post->status . '">';
                    };
                    ...
                    if ($check_module == 'panda_prdncn') {
                        ...
                        $nestedData['status'] = $post->status;
                        $nestedData['stock_collected_by'] = $post->stock_collected_by;
                        ...
                        //$nestedData['button'] = "";//
                        $nestedData['batch_no'] = "<a href=" . site_url('panda_return_collection/return_collection_child') . "?refno=" . $post->batch_no . "&loc=" . $_REQUEST['loc'] . "&pc=" . substr($post->strb_doc_date,0,7) . " >" . $post->batch_no . "</a>";
                        $nestedData['uploaded_image'] = $post->uploaded_image;
                    };
                    ...
                    if ($check_module == 'panda_pci') {
                        ...
                        $nestedData['status'] = $post->status;
                        if ($this->session->userdata('customer_guid') != '1F90F5EF90DF11EA818B000D3AA2CAA9' && $this->session->userdata('customer_guid') != '907FAFE053F011EB8099063B6ABE2862' && $this->session->userdata('customer_guid') != 'D361F8521E1211EAAD7CC8CBB8CC0C93' && $this->session->userdata('customer_guid') != '403810171FA711EA9BB8E4E7491C3E1E' 
                    ...
                    {*/
                        ...
                        $nestedData['expiry_date'] = $post->expiry_date;
                        if($post->srb_accept_days != '0')
                        {
                            $dateto = date_create($post->new_expiry_date);
                            $nestedData['expiry_date'] = $post->new_expiry_date;
                        }
                        /* }
                    ...
                    else
                    {
                        ...
                        if ($datefrom > $dateto) {
                            if ($post->status != 0) {
                                $nestedData['canceled'] = "<span style='color:red;font-weight:bold;'>-</span>";
                            } else {
                                $nestedData['canceled'] = "<span style='color:red;font-weight:bold;'>-" . $interval->format('%a') . "</span>";
                            }
                        } else {
                            if ($post->status != 0) {
                                $nestedData['canceled'] = "<span style='color:red;font-weight:bold;'>-</span>";
                        ...
                        if($post->b2b_registration == '0')
                        {
                            $nestedData['canceled'] = "<span style='color:red;font-weight:bold;'>-</span>";
                        }

                        if($post->accepted_at == '0000-00-00 00:00:00' || $post->accepted_at == '' || $post->accepted_at == 'null' || $post->accepted_at == null)
                        {
                            $nestedData['accepted_at'] = '';
                            $nestedData['accepted_by'] = '';
                        }
                        else
                        {
                            $nestedData['accepted_at'] = $post->accepted_at;
                            $nestedData['accepted_by'] = $post->accepted_by;
                        }

                        $nestedData['cancel_remark'] = $post->cancel_remark;
                        if($post->uploaded_image == '1')
                        {
                            $nestedData['button'] =  "<a href=" . site_url('panda_return_collection/return_collection_child') . "?refno=" . $post->batch_no . "&loc=" . $_REQUEST['loc'] . "&pc=" . substr($post->doc_date,0,7) . " style='float:left;margin-left:5px;' class='btn btn-sm btn-info' role='button'><span class='glyphicon glyphicon-eye-open'></span></a>
                            <button style='margin-left:5px;' id='btn_image' type='button'  title='IMAGE' class='btn btn-sm btn-warning' refno=" . $post->batch_no . " period_code=" . $post->doc_date . " outlet=" . $post->location . " image_type='STRB'><i class='fa fa-file-image-o'></i></button>";
                        }
                        else
                        {   
                            $nestedData['button'] =  "<a href=" . site_url('panda_return_collection/return_collection_child') . "?refno=" . $post->batch_no . "&loc=" . $_REQUEST['loc'] . "&pc=" . substr($post->doc_date,0,7) . " style='float:left;margin-left:5px;' class='btn btn-sm btn-info' role='button'><span class='glyphicon glyphicon-eye-open'></span></a>";
    ...
    public  function po_filter()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $this->session->userdata('user_logs') == $this->panda->validate_login()) {
            ...
            } else {   //initial idea is to have default 7 days from today
    ...
    public function accept()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $this->session->userdata('user_logs') == $this->panda->validate_login()) {
            ...
            $check_postatus = $this->db->query("SELECT * from b2b_summary.pomain_info where refno = '$refno' and customer_guid = '$customer_guid'");
            ...
            $this->db->query("UPDATE b2b_summary.pomain_info set status = 'Accepted',b2b_status = 'readysend' where customer_guid ='$customer_guid' and refno = '$refno'");
        ...
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $this->session->userdata('user_logs') == $this->panda->validate_login()) {
            ...
            if ($from_module == 'panda_return_collection') // return collection accept collection detail
            {
                ...
                if ($check_status->row('status') == '1') {
                    ...
                    $this->db->query("UPDATE b2b_summary.dbnote_batch set status = '1' , accepted_by = '$user_name', accepted_at =  now(),uploaded = 0,action_date = CURDATE() where customer_guid ='$customer_guid' and batch_no = '$refno' and status = '0'");
                    $this->db->query("REPLACE into supplier_movement select 
                        upper(replace(uuid(),'-','')) as movement_guid
                        , '$customer_guid'
                        , '$user_guid'
    ...
    public function reject()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $this->session->userdata('user_logs') == $this->panda->validate_login()) {
            ...
            if ($from_module == 'b2b_po') {
                $refno_loc_group = $this->db->query("SELECT * FROM $b2b_database.pomain_info WHERE RefNo = '$refno' AND customer_guid = '$customer_guid' ");
                $refno_loc_group_code = $refno_loc_group->row('loc_group');
                $refno_customer_guid = $refno_loc_group->row('customer_guid');
                $refno_supplier_name = $refno_loc_group->row('supplier_name');
                $check_postatus = $this->db->query("SELECT * from b2b_summary.pomain_info where refno = '$refno' and customer_guid = '$customer_guid' and status in ('', 'viewed', 'printed')");
                ...
                $this->db->query("UPDATE b2b_summary.pomain_info set status = 'rejected', rejected_remark = '$reason' , rejected = '1' , b2b_status = 'readysend' ,rejected_at = now()  where customer_guid ='$customer_guid' and refno = '$refno'");
            };
            ...
            $get_mail_template = $this->db->query("SELECT mail_subject,body_content FROM lite_b2b.email_template WHERE is_active = '1' AND `type` = 'PO_RJT' AND mail_type = 'PO_RJT'");
            ...
            $customer_name = $this->db->query("SELECT * FROM acc WHERE acc_guid = '$customer_guid' LIMIT 1")->row('acc_name');
            ...
            $replace_key = ['%refno%','%now%','%acc_name%','%supplier_name%','%user_id%','%outlet%','%rejected_remark%'];
            $replace_value = [$refno,$datetimenow,$customer_name,$refno_supplier_name,$user_id,$refno_loc_group_code,$email_reject_reason];
            ...
            foreach ($email_group->result() as $row) {
                ...
                $email_add = 'procurement@suiwah.com.my';

                $return_send_final = $this->Send_email_model->send_mailjet_third_party($email_add, '', $bodyContent, $email_subject, '', '' , '','no-reply@sunshineonline.com.my','');
            }
            // end
    ...
    public function merge_pdf()
    {
        ...
        foreach ($list_id as $row) {
            ...
            $type = $parameter->row('type');
            $code = $check_scode;
            $code = str_replace(" ", "%20", $check_scode);
            ...
            $virtual_path = $this->db->query("SELECT CONCAT(file_host,file_path) AS file_path FROM acc WHERE acc_guid = '" . $_SESSION['customer_guid'] . "'")->row('file_path');
            //echo $this->db->last_query(); die;
            ...
            // echo $file_config_final_path;die; MPMP
            // $file_config_final_path = 'D:\\';

            $old_pdf = $virtual_path . $path_seperator . $replace_var . '.pdf';

            $savedpdf = $file_config_final_path . $path_seperator . $replace_var . '.pdf';

            copy($old_pdf, $savedpdf);

            //after change to json remove comment...
        ...
        {   
            ...
            $link_url = site_url($frommodule . '/direct_print_merge?trans=&loc=' . $loc . '&pdfname=' . $pdf_name); 
        }
        else
        {   
    ...
    public function merge_pdf_prdncn()
    {
        ...
        foreach ($list_id as $row) {
            ...
            $path_seperator = $this->file_config_b2b->path_seperator($customer_guid, 'web', 'general_doc', 'path_seperator', 'PS');
            $file_config_final_path = $this->file_config_b2b->file_path_name($customer_guid, 'web', 'general_doc', 'merge_print', 'MRGPRT');
            // echo $file_config_final_path;die;
    ...
    public function acc_doc_merge_pdf()
    {
        ...
        foreach ($list_id as $row) {
            ...
            $file_config_final_path = $this->file_config_b2b->file_path_name($customer_guid, 'web', 'general_doc', 'merge_print_acc_doc', 'ACCMRGPRT');

            $uid = $code . '_' . str_replace('/', '', $row);
            $rute = $file_config_final_path . '/' . $uid . '.pdf';
            //$rute = 'merge/'.$uid.'.pdf';
            //print_r($rute); die;
            ...
            array_push($temp_file, $file_config_final_path . '/' . $code . '_' . $row . '.pdf');
        }
        $xxrefno = rtrim($xrefno, ",");
        ...
        $pdf_file = $pdf_name;
        $pdf->merge('file', $merge_create_path . $path_seperator . $pdf_name . '.pdf'); // generate the file
        //array_push($temp_file,$file_config_final_path.'/'.$pdf_name.'.pdf'); //unlink merge file
        foreach ($temp_file as $key => $val) {
            if (file_exists($val)) {
                unlink($val);
            }
    ...
    public function merge_jasper_pdf()
    {
        // ob_start();
        ...
        foreach ($list_id as $count => $row) {
            if ($type == 'PO') {
                ...
                $url = $this->jasper_ip . "/jasperserver/rest_v2/reports/reports/PandaReports/Backend_GRN/gr_supplier_copy.pdf?refno=".$row; // po
                ...
                $url = $this->jasper_ip . "/jasperserver/rest_v2/reports/reports/PandaReports/Backend_GRN/GRDA.pdf?refno=".$row; // po
                $frommodule = 'b2b_grda';
            } else if ($type == 'PRDNCN') {
                ...
                if($get_doc_type == 'CN')
                {
                    $url = $this->jasper_ip . "/jasperserver/rest_v2/reports/reports/PandaReports/Backend_PRCN/main_jrxml.pdf?refno=".$row;
                    $frommodule = 'b2b_prcn';
                }
                else 
                {
                    $url = $this->jasper_ip . "/jasperserver/rest_v2/reports/reports/PandaReports/Backend_PRDN/main_jrxml.pdf?refno=".$row;
                    $frommodule = 'b2b_prdn';
                }
            ...
            if($httpcode != '400')
            {
                ...
                if($type == 'PO') 
                {
                    ...
                    {                    
                        $this->db->query("UPDATE b2b_summary.pomain_info set status = 'printed' where customer_guid ='$customer_guid' and refno = '$row' and status = '' ");
                ...
                {
                    if (!in_array('!SUPPMOV', $_SESSION['module_code'])) {
                        $this->db->query("UPDATE b2b_summary.grmain_info set status = 'printed' where customer_guid ='$customer_guid' and refno = '$row' and status IN ('','viewed') ");
                ...
                else if ($type == 'GRDA') 
                {
                    ...
                    {
                        $this->db->query("UPDATE b2b_summary.grmain_dncn_info set status = 'printed' where customer_guid ='$customer_guid' and refno = '$row' and status IN('','viewed') ");
                ...
                {
                    if($frommodule == 'b2b_prdn')
                    {
                        if (!in_array('!SUPPMOV', $_SESSION['module_code'])) {
                            $this->db->query("UPDATE b2b_summary.dbnotemain_info set status = 'printed' where customer_guid ='$customer_guid' and refno = '$row' and status IN ('','viewed') ");
                    ...
                    {
                        if (!in_array('!SUPPMOV', $_SESSION['module_code'])) {
                            $this->db->query("UPDATE b2b_summary.cnotemain_info set status = 'printed' where customer_guid ='$customer_guid' and refno = '$row' and status IN ('','viewed') ");
                ...
                {
                    if (!in_array('!SUPPMOV', $_SESSION['module_code'])) {
                        $this->db->query("UPDATE b2b_summary.cndn_amt_info set status = 'printed' where customer_guid ='$customer_guid' and refno = '$row' and status IN ('','viewed') ");
    ...
    public function strb_show_image_logs()
    {
        $customer_guid = $this->session->userdata("customer_guid");
        $user_guid = $_SESSION['user_guid'];        
        $from_module = $_SESSION['frommodule'];
