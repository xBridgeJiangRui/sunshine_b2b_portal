...
class Consignment_report extends CI_Controller
{
        ...
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            if (!isset($_REQUEST['Date_From']) || !isset($_REQUEST['Date_To'])) {
                ...
                if ($diff->format("%R%a days") > 30) {
                    echo 'Date Range Exceed 31 days';
                    die;
                };
            }
    ...
    public function consignment_sales_statement_child()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            $trans_guid = str_replace(" ", "%20",$_REQUEST['trans']);
            // echo $trans_guid;die;
            $customer_guid = $this->session->userdata('customer_guid');
            $user_guid = $this->session->userdata('user_guid');
            $query_loc = $this->session->userdata('query_loc');
    ...
    public function consignment_sales_statement_view()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            $refno = rawurlencode($consignment_sales_statement_refno);

            $path_seperator = $this->file_config_b2b->path_seperator($customer_guid, 'web', 'general_doc', 'path_seperator', 'PS');

            $jasper_url = $this->file_config_b2b->file_path_name($customer_guid, 'web', 'consignment', 'consignment_jasper_ip', 'CONJASPERIP');
        ...
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            if ($check_e_invoices > 0) {
                ...
                if ($output->status == "true") {
                    ...
                    redirect(site_url('Consignment_report/consignment_sales_statement_child') . '?trans=' . $trans_guid . '&loc=' . $loc . '&period_code=' . $period_code . '&status=' . $status . '&date_trans=' . $date_trans);
                } else {
                    $check_e_invoices = count($output->result);
    ...
    public function consignment_report()
    {
        ...
        if (isset($_REQUEST['Date_From']) && isset($_REQUEST['Date_To']) && isset($_REQUEST['main_loc']) && isset($_REQUEST['Consign_code'])) {
            ...
            if ($ver == 0) {
                ...
                // for sku cs date testing data at b2b database
                // if($user_guid == '7BA14C79BDDB11EBB0C4000D3AA2838A')
                // {
                //     $db_be = 'backend';
                //     $db_re = 'compare_bataras';
                //     $db_su = 'b2b_summary';
                //     $jasper_url = $this->file_config_b2b->file_path_name($customer_guid,'web','general_doc','jasper_invoice_ip','GDJIIP');
            ...
            // if($user_guid == '7BA14C79BDDB11EBB0C4000D3AA2838A')
            // {
            //     // $report_ip = $jasper_url.'/jasperserver/rest_v2/reports/reports/B2BReports/'.$report_template.'.pdf'.'?Date_From='.$Date_From.'&Date_To='.$Date_To.'&db_be='.$db_be.'&db_re='.$db_re.'&supcode='.$Consign_code.$main_loc.'&db_su='.$db_su.'&acc_guid='.$customer_guid;
            //     echo $report_ip;die;
            // }
    ...
    public function consignment_redirect_location_by_supcode()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            // echo 1;die;
            // echo $this->input->post('location');die;
            $consign_supplier = $this->input->post('consign_supplier');
            
            if(isset($_REQUEST['f_t'])) // period to fliter
            {
                $to_period_code = $_REQUEST['f_t'];
            }
            else
            {
                $to_period_code = '';
            ...
            redirect('Consignment_report/consignment_sales_statement_by_supcode?status=' . $status . '&loc=' . $loc . '&period_code=' . $period_code.'&f_t='.$to_period_code.'&f_s='.$consign_supplier);
        } else {
            $this->session->set_flashdata('message', 'Session Expired! Please relogin');
            redirect('#');
        }
    ...
    public function consignment_sales_statement_by_supcode()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            // if($_SESSION['customer_guid'] == '1F90F5EF90DF11EA818B000D3AA2CAA9' && ($_SESSION['user_guid'] != '9B5F07FE2BF011E8820A543D3AA2838A' && $_SESSION['user_guid'] != '6615820DB41111E9A7FA000D3AA2838A' && $_SESSION['user_guid'] != '8941E3B5BDDB11EBB0C4000D3AA2838A' && $_SESSION['user_guid'] != '7BA14C79BDDB11EBB0C4000D3AA2838A' && $_SESSION['user_guid'] != 'B194913010BC11EA8675000D3AA2838A'))
            // {
            //     echo "<script> alert('Sorry Under Maintenance.'); 
            //     document.location='" . base_url() . "index.php/dashboard'</script>";
            //     return;
            // }
            ...
            if(isset($_REQUEST['f_s'])) // filter_supplier
            {
                $filter_supplier = $_REQUEST['f_s'];
            }
            else
            {
                $filter_supplier = '';
            ...
            if(isset($_REQUEST['f_t'])) // filter period to 
            {
                $tperiod_code = $_REQUEST['f_t'];
            }
            else
            {
                $tperiod_code = '';
            ...
            // get supplier details script here@@@@@
            $url = $this->api_url;

            $to_shoot_url = $url."/Select/S_get_consignment_supplier_data";
            // $block = $this->db->query("SELECT * FROM set_setting WHERE module_name = 'CONSIGNMENT' AND code = 'CONS' LIMIT 1");
            $data = array(
                'customer_guid' => $customer_guid,
                'user_guid' => $user_guid,
    ...
    public function view_table_by_supcode()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            if(isset($_REQUEST['f_t'])) // for date to
            {
                $tperiod_code = $_REQUEST['f_t'];
            }
            else
    ...
    public function consignment_sales_statement_by_supcode_list()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            $datatable_url = site_url('Consignment_report/view_table_by_supcode_list?status=' . $status . '&loc=' . $loc . '&period_code=' . $speriod_code . '&supcode=' . $ssupcode . '&date_trans=' . $date_trans . '&company_id=' . $company_id);
            ...
            if ($output->status == "true") {
                ...
                $inv_generate_count = $output->count_generated;
            } else {
                $consignment_sales_statement_header = $output->result;
                $consignment_sales_statement_sup_inv_no = $consignment_sales_statement_header[0]->sup_doc_no;
                $consignment_sales_statement_status = $consignment_sales_statement_header[0]->status;
            ...
            $target_dir = $file_path_name . $path_seperator . $customer_guid . $path_seperator . 'consign_inv' . $path_seperator . $target_dir_supplier_guid . $path_seperator . $ssupcode . '_' . $speriod_code . '.pdf';
            // $file_type = '.pdf';            
            // echo $target_dir;die;
            ...
            $limit_date = $consignment_sales_statement_header['0']->date_to;
            $limit_date_next = $this->db->query("SELECT DATE_FORMAT(DATE_ADD('$limit_date', INTERVAL 0 MONTH),'%Y-%m-01') as date")->row('date');
            // echo $limit_date_next;die; 
            $data = array(
                'consignment_sales_statement_header' => $consignment_sales_statement_header,
                'limit_date' => $limit_date_next,
                'file_upload_supcode' => $ssupcode . '_' . $speriod_code,
                'file_upload_supplier_guid' => $target_dir_supplier_guid,
                'file_upload_type' => 'consign_inv',
                // 'show_consignment_e_invoice' => $show_consignment_e_invoice,
                // 'show_reject' => $show_reject,
                'consignment_sales_statement_sup_code' => $consignment_sales_statement_sup_code,
                'consignment_sales_statement_sup_inv_no' => str_replace('&','%26',str_replace('#','%23',$consignment_sales_statement_sup_inv_no)),
                'consignment_sales_statement_status' => $consignment_sales_statement_status,
                'show_consignment_e_invoice' => 0,
                'show_reject' => '',
                'datatable_url' => $datatable_url,
                ...
                'inv_generate_count' => $inv_generate_count,
            );
    ...
    public function consignment_e_invoice_by_supcode_view()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            $user_guid = $this->session->userdata('user_guid');
            $report_type = $_REQUEST['report_type'];
            //$trans_guid = str_replace('&','%26',str_replace('#','%23',str_replace(' ','%20',$_REQUEST['trans_guid'])));
            $trans_guid = rawurlencode($_REQUEST['trans_guid']);
            $supcode = str_replace(" ", "%20", $_REQUEST['supcode']);
            $company_id = $_REQUEST['company_id'];
            // echo $report_type;die;
            // echo $consignment_sales_statement_refno;die;
    ...
    public function header_save_inv_no_by_supcode()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            ...
            $date_trans = $this->input->post('date_trans');
            $company_id = $this->input->post('company_id'); // new add 03-08-2022
            ...
            $check_supplier_inv_no = $this->db->query("SELECT b2b_inv_no FROM $database2.`acc_trans` WHERE b2b_inv_no = '$b2b_inv_no' AND supcus_code != '$submit_supcode' GROUP BY supcus_code");
            if ($check_supplier_inv_no->num_rows() > 0) {
                $data = array(
                    'para1' => 1,
                    'msg' => 'E invoices not generated, Supplier Invoice No Cannot Duplicate.',
                );
            ...
            $data = array(
                ...
                'company_id' => $company_id,  // new add 03-08-2022              
            );
    ...
    //app mobile function Doc_info
    public function consignment_generate_e_invoices_by_supcode()
    {
        if ($this->session->userdata('loginuser') == true && $this->session->userdata('userid') != '' && $_SESSION['user_logs'] == $this->panda->validate_login()) {
            $b2b_inv_no = trim($this->input->post('b2b_inv_no'));
            $consign_invoice_date = $this->input->post('consign_invoice_date');
            $loc = $this->input->post('loc');
            ...
            $company_name = $this->db->query("SELECT IFNULL(company_name,'') AS company_name FROM $database2.`acc_trans` WHERE supcus_code = '$supcode' AND company_id = '$company_id' GROUP BY supcus_code")->row('company_name');
            // end new add 03-08-2022

            // $query_loc = $this->session->userdata('query_loc');

            if (($consign_invoice_date == '') || ($consign_invoice_date == 'null') || ($consign_invoice_date == null)) {
                $this->session->set_flashdata('warning', 'E invoices not generated, Supplier Invoice Date Cannot Empty');
                redirect(site_url('Consignment_report/consignment_sales_statement_by_supcode_list') . '?status=' . $status . '&loc=' . $loc . '&period_code=' . $period_code . '&supcode=' . $supcode . '&date_trans=' . $date_trans . '&company_id=' . $company_id);
            }

            if(strpos($b2b_inv_no, ',') !== false ) 
            {
                $this->session->set_flashdata('warning', 'E invoices not generated, Supplier Invoice Cannot have Comma');
                redirect(site_url('Consignment_report/consignment_sales_statement_by_supcode_list').'?status='.$status.'&loc='.$loc.'&period_code='.$period_code.'&supcode='.$supcode.'&date_trans='.$date_trans.'&company_id='.str_replace(' ','%20',$company_id));
            }

            // if(strpos($b2b_inv_no, '/') !== false ) 
            // {
            //     $this->session->set_flashdata('warning', 'E invoices not generated, Supplier Invoice Cannot have Slash');
            //     redirect(site_url('Consignment_report/consignment_sales_statement_by_supcode_list').'?status='.$status.'&loc='.$loc.'&period_code='.$period_code.'&supcode='.$supcode.'&date_trans='.$date_trans.'&company_id='.str_replace(' ','%20',$company_id));
            // }

            if(strlen($b2b_inv_no) > '30')
            {
                $this->session->set_flashdata('warning', 'E invoices not generated, Supplier Invoice length exceed limit 30 characters.');
                redirect(site_url('Consignment_report/consignment_sales_statement_by_supcode_list').'?status='.$status.'&loc='.$loc.'&period_code='.$period_code.'&supcode='.$supcode.'&date_trans='.$date_trans.'&company_id='.str_replace(' ','%20',$company_id));
            }

            $database2 = $this->db->query("SELECT b2b_database FROM $database1.acc WHERE acc_guid = '$customer_guid'")->row('b2b_database');

            //AND supcus_code != '$supcode' 
            $check_supplier_inv_no = $this->db->query("SELECT b2b_inv_no FROM $database2.`acc_trans` WHERE b2b_inv_no = '$b2b_inv_no' AND company_id = '$company_id' AND `status` = 'Invoice Generated' GROUP BY supcus_code");

            if ($check_supplier_inv_no->num_rows() > 0) {
                $this->session->set_flashdata('warning', 'E invoices not generated, Supplier Invoice No Cannot Duplicate');
                redirect(site_url('Consignment_report/consignment_sales_statement_by_supcode_list') . '?status=' . $status . '&loc=' . $loc . '&period_code=' . $period_code . '&supcode=' . $supcode . '&date_trans=' . $date_trans . '&company_id=' . $company_id);
            }
    ...
    public function reset_consignment_sales()
    {
        ...
        if ($check_e_invoices_hub->num_rows() == 0) {
            $data = array(
                'para1' => 1,
                'msg' => 'Empty Data Consignment E-Invoice Hub.',
            );
            echo json_encode($data);
            die;
        }
