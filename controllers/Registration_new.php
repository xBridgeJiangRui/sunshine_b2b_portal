...
class registration_new extends CI_Controller
{
    ...
    if ($this->session->userdata('loginuser') == true) {
      ...
      foreach ($user_details->result() as $row) {
        ...
        if ($check_reset_link->num_rows() > 0) {
          $reset_url = 'https://sunshineb2b.com.my/index.php/Key_in/key_in?si=' . $check_reset_link->row('reset_guid') . '&ug=' . $check_reset_link->row('user_guid');
    ...
    if (!empty($posts)) {
      foreach ($posts as $post) {
        ...
        if ($_SESSION['user_group_name'] == 'SUPER_ADMIN' || $_SESSION['user_group_name'] == 'CUSTOMER_ADMIN_TESTING_USE') {
          ...
          if ($_SESSION['customer_guid'] == '13EE932D98EB11EAB05B000D3AA2838A' || $_SESSION['customer_guid'] == '610BB0EA76AE11EDB37C72B64FC54D79') {
  ...
  public function register_form_edit_new()
  {
    if ($this->session->userdata('loginuser') == true) {
      ...
      foreach ($user_details->result() as $row) {
        ...
        $vendor_code_query = $this->db->query("SELECT * FROM lite_b2b.set_supplier_group a WHERE a.supplier_guid = '$supplier_guid' AND a.customer_guid = '$customer_guid'");
        // echo $this->db->last_query();die;
        $vendor_code_dropdown = '<select style="width:100%" class="selectpicker" multiple id="detail_option'.$countid.'">';
        $mapped_string = '';
        foreach ($vendor_code_query->result() as $dropdown) {
          if (in_array($dropdown->supplier_group_guid, $selected_vendor_code_array)) {
            $selected = 'selected';
  ...
  public function send_mail()
  {
    ...
    if (($register_form_details->num_rows() == '1') && ($type == 'register')) {
      ...
      $store_logo = 'https://sunshineb2b.com.my/asset' . $store_logo . '/' . $get_pic . '.jpg'; // need change path
      ...
      $url = 'https://sunshineb2b.com.my/index.php/Supplier_registration/register_form_edit?link=' . $register_guid;
      ...
      $this->Send_email_model->send_mailjet_third_party($email_add, '', $bodyContent, $subject, '', '', '', 'no-reply@sunshineonline.com.my','');
    ...
    if (($vendor_form_details->num_rows() == '1') && ($type == 'vendor')) {
      ...
      $url = 'https://sunshineb2b.com.my/index.php/Supplier_registration/vendor_form_edit?link=' . $register_guid;
  ...
  public function edit_reg_app_vendor()
  {
    ...
    $edit_form_status = $this->input->post('edit_form_status');
    $user_id = $this->db->query("SELECT a.user_id FROM set_user a WHERE a.user_guid ='" . $_SESSION['user_guid'] . "'")->row('user_id');
    $updated_at = $this->db->query("SELECT now() as now")->row('now');
  ...
  public function proceed_vendor()
  {
    ...
    foreach ($vendor_code1 as $supplier_group_name) {
      ...
      if ($check_name->num_rows() <= 0) {
        ...
        $ver_sunshine = '0'; //$email_group->num_rows() > 0
        if ($ver_sunshine == 'disabled') {
          ...
          $bodyContent = '<div class="container-fluid">
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <p class="lead">
                                        Following suppliers has registered @ Sunshine B2B Portal for Retailer: ' . $customer_name->row('acc_name') . '<br>
                                            ...
                                            Regards,<br>
                                            <a href="' . $url . '"> Sunshine B2B Portal</a>
                                        </p>
                                    </div>
                                </div>
                            </div>';
  ...
  public function send_mailjet_third_party($email_add, $date, $bodyContent, $email_subject, $module, $cc_list_string, $pdf, $reply_to)
  {
    ...
    //10.0.0.251
    $to_shoot_url = "20.212.51.33/pandaapi3rdparty/index.php/send_mail/blast_email_api";
    ...
    $data = array(
      ...
      "to_email" => $email_add,
      "cc_list_string"  => $cc_list_string,
      "pdf" => $attachment_array,
    );
    //print_r($data); die;
  ...
  public function send_mailjet_third_party_backup($email_add, $date, $bodyContent, $email_subject, $module, $cc_list_string, $pdf, $reply_to)
  {
    // die;
    if ($pdf != '' || $pdf != null) {
      $b64Doc = chunk_split(base64_encode(file_get_contents($pdf)));
  ...
  public function email_subs_function()
  {
    ...
    foreach ($details as $row) {
      if ($row->duplicate == 0) {
        ...
        $reset_url = 'https://sunshineb2b.com.my/index.php/Key_in/key_in?si=' . $reset_link->row('reset_guid') . '&ug=' . $reset_link->row('user_guid');
  ...
  {
    $register_guid = $this->input->post('register_guid');
    $register_child = $this->db->query("SELECT a.*, GROUP_CONCAT(b.`register_mapping_guid` ORDER BY b.`mapping_type` DESC) AS register_mapping_guid, GROUP_CONCAT(b.`mapping_type` ORDER BY b.`mapping_type` DESC) AS mapping_type, GROUP_CONCAT(b.`ven_agency`) AS ven_agency, GROUP_CONCAT(b.`ven_code`) AS ven_code, c.`customer_guid`, GROUP_CONCAT(b.`vendor_code_remark`) AS vendor_code_remark, c.`form_status` FROM lite_b2b.register_child_new a LEFT JOIN lite_b2b.`register_child_mapping` b ON a.`register_c_guid` = b.`register_c_guid` INNER JOIN lite_b2b.register_new c ON a.`register_guid` = c.`register_guid` WHERE a.`register_guid` = '$register_guid' AND part_type = 'registration' GROUP BY a.register_c_guid ORDER BY created_at ASC");
    //echo $this->db->last_query(); die;
